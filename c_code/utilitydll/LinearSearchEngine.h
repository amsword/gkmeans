...
namespace utility
{
	...
	template <class T>
	class LinearSearchLargest : public SearchEngine
	{
	public:
		void SetNumberCandidate(int num_candidate)
		{
			m_nNumCandidate = num_candidate;
		}
		void Initialize(const SMatrix<T> * pDataBase, DistanceCalculator* p_dist)
	...
	protected:
		void PopFromHeap(Heap<LessPair<double> >& heap, int* p_idx)
		{
			int k = heap.size() - 1;
		...
		void FindBestCandidate(void* p_query, Heap<LessPair<double> >& heap_knn) const
		{
			for (int j = 0; j < m_nNumDataBasePoint; j++)
			{
				LessPair<double> node;
		...
		void FindBestCandidatePre(void* p_query, Heap<LessPair<double> >& heap_knn) const
		{
			for (int j = 0; j < m_nNumDataBasePoint; j++)
			{
				LessPair<double> node;
		...
		void FindBestCandidate(const QueryPreprocessing& query, Heap<LessPair<double> >& heap_knn) const
		{
			for (int j = 0; j < m_nNumDataBasePoint; j++)
			{
				LessPair<double> node;

	...
	int num_partitions)
{
...

	{
		...
		SMART_ASSERT(squared_distance(vec.Ptr(), m_vecBucketNumber.Ptr(), m_nNumberPartitions) == 0).Exit();
	}
	...
	SMART_ASSERT(m_vecBucketNumber.SumUp() == m_pmatLookup->Rows() && 
		m_vecBucketNumber.SumUp() == m_pmatLookup->Cols())
		(m_vecBucketNumber)
		(m_vecBucketNumber.SumUp()).Exit();
